...
function selectTodo(start_date) {
  db.serialize(function () {
    ...
    //データを取得する
    db.get(
      "SELECT title,discription,start_date,end_date FROM todo where ${start_date} between CURRENT_DATE and date('now', '1 days') order by start_date",
      function (err, row) {
        if (err) {
          return;
        } else {
...
function btnCreate_click() {
  showMessageBox("作成してもよろしいですか？", "info");
}
